...
--   3. date_end   = (date_planned + temps_immobilisation secondes), snappée à 00:00:00 du jour
--      Si temps_immobilisation est NULL ou 0 -> date_end = date_start (même jour, 00:00:00)
--
-- Les dates sont stockées sans heure (00:00:00). Le module utilise ensuite
-- [date_start, date_end] pour afficher les barres du Gantt dans le planning
-- Atelier (workshop_planning.php).
-- ----------------------------------------------------------------------------
...
SET
    ...
    date_end   = CASE
                     WHEN temps_immobilisation IS NULL OR temps_immobilisation = 0
                         THEN DATE_FORMAT(date_planned, '%Y-%m-%d 00:00:00')
                     ELSE DATE_FORMAT(
