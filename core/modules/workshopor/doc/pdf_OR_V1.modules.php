...
/**
 *	\file       workshop/core/modules/workshopor/doc/pdf_OR_V1.modules.php
 *	\ingroup    workshop
 *	\brief      Template PDF V1 pour les Ordres de Réparation Workshop
 */
...
/**
 *	Class to build PDF documents for Workshop Repair Orders (OR)
 */
class pdf_OR_V1 extends ModelePDFWorkshopOR
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$langs->loadLangs(array('workshop@workshop', 'orders', 'companies', 'bills', 'products', 'other'));
		$this->db = $db;
		$this->name = 'OR_V1';
		$this->description = "Document OR avec ajout des documents obligatoires";
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (! empty(getDolGlobalString("MAIN_USE_FPDF"))) $outputlangs->charset_output='ISO-8859-1';
		...
		if (! empty(getDolGlobalString("MAIN_GENERATE_ORDERS_WITH_PICTURE"))) {
			...
			for ($i = 0 ; $i < $nblines ; $i++) {
				...
				if (! empty(getDolGlobalString("PRODUCT_USE_OLD_PATH_FOR_PHOTO"))) {
				...
				foreach ($pdir as $midir) {
					if (! $arephoto) {
						...
						foreach ($objphoto->liste_photos($dir, 1) as $key => $obj) {
							if (empty(getDolGlobalString("CAT_HIGH_QUALITY_IMAGES"))) {		// If CAT_HIGH_QUALITY_IMAGES not defined, we use thumb if defined and then original photo
		...
		if ($conf->workshop->dir_output) {
			...
			if ($object->specimen) {
				...
				$dir = $conf->workshop->multidir_output[$object->entity] . "/" . $objectref;
			...
			if (file_exists($dir)) {
				...
				$heightforfreetext = (isset($conf->global->MAIN_PDF_FREETEXT_HEIGHT) ? getDolGlobalInt("MAIN_PDF_FREETEXT_HEIGHT")  : 5); // Height reserved to output the free text on last page
				$heightforfooter = $this->marge_basse + (empty(getDolGlobalInt("MAIN_GENERATE_DOCUMENTS_SHOW_FOOT_DETAILS")) ? 10 : 15); // Height reserved to output the footer (value include bottom margin)
				...
				if (! empty(getDolGlobalString("MAIN_ADD_PDF_BACKGROUND"))) {
					$pagecount = $pdf->setSourceFile($conf->mycompany->multidir_output[$object->entity].'/'.getDolGlobalInt("MAIN_ADD_PDF_BACKGROUND"));
				...
				if (! empty(getDolGlobalString("MAIN_ADD_SALE_REP_SIGNATURE_IN_NOTE"))) {
					// Get first sale rep
				...
				if ($notetoshow) {
					...
					if ($pageposafternote > $pageposbeforenote) {
						...
						while ($pagenb < $pageposafternote) {
							...
							$pagenb++;
							if (!empty($tplidx)) $pdf->useTemplate($tplidx);
							if (empty(getDolGlobalString("MAIN_PDF_DONOTREPEAT_HEAD"))) $this->_pagehead($pdf, $object, 1, $outputlangs);
				...
				for ($i = 0 ; $i < $nblines ; $i++) {
					...
					if ($this->getColumnStatus('photo')) {
						...
						if (isset($imglinesize['width']) && isset($imglinesize['height']) && ($curY + $imglinesize['height']) > ($this->page_hauteur-($heightforfooter+$heightforfreetext+$heightforinfotot))) {	// If photo too high, we moved completely on new page
							...
							if (!empty(getDolGlobalString("MAIN_PDF_DATA_ON_FIRST_PAGE")))
								$showpricebeforepagebreak = 1;
					...
					if ($this->getColumnStatus('ean')) {
						...
						if (empty($object->lines[$i]->fk_product) || empty($this->cacheOrScan[$object->lines[$i]->fk_product])) {
							...
							$barcodetype=empty(getDolGlobalString("OPERATION_ORDER_BARCODE_TYPE"))?'C128':getDolGlobalString("OPERATION_ORDER_BARCODE_TYPE");
					...
					if ($localtax1_type && $localtax1ligne != 0)
						...
						if ($localtax2_type && $localtax2ligne != 0)
							...
							// Add line
					if (! empty(getDolGlobalString("MAIN_PDF_DASH_BETWEEN_LINES")) && $i < ($nblines - 1)) {
						$pdf->setPage($pageposafter);
					...
					if (isset($object->lines[$i + 1]->pagebreak) && $object->lines[$i + 1]->pagebreak) {
						...
						if (empty(getDolGlobalString("MAIN_PDF_DONOTREPEAT_HEAD"))) $this->_pagehead($pdf, $object, 1, $outputlangs);
							...
							//On detecte si le produit de la ligne est attaché a un document Obligatoire
					If (!empty($object->lines[$i]->product->array_options['options_doc_obl'])) {
						$moredoc[$object->lines[$i]->product->ref] = $object->lines[$i]->product->array_options['options_doc_obl'];
					}
				}
				...
				if ($pagenb == $pageposbeforeprintlines)
					$this->_tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, $hidetop, 0, $object->multicurrency_code);
				else $this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, $hidetopNewPage, 0, $object->multicurrency_code);
						$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
						$pdf->Rect($this->marge_gauche, $bottomlasttab, $this->page_largeur-$this->marge_droite-$this->marge_gauche, 25);
						$out = $outputlangs->transnoentities('Customer_Sign');
						$out = $outputlangs->convToOutputCharset($out);
						$pdf->SetFont('helvetica', 'BU', $default_font_size);
						$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->marge_gauche, 0, $out, 0, 'L', false, 1, $this->marge_gauche+1, $bottomlasttab+1);
						...
						//on boucle sur les doc obligatores et on les insères dans le fichier pdf a la suite de l'OR

				if (!empty($object->array_options['options_doc_ans'])) {
							$docans = unserialize(html_entity_decode($object->array_options['options_doc_ans']));
					$nb= count($docans)-1;
					//print_r ($nb); exit;
					for ($i =0 ; $i<=$nb ; $i++) {
						$this->add_doc($object, $outputlangs, $pdf, $i, '');
					}
				} else {
					foreach ($moredoc as $key => $value) {
						$this->add_doc($object, $outputlangs, $pdf, $key, $value);
					}
				}
						$pdf->Close();
						$pdf->Output($file, 'F');
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop)) {
			//getDolGlobalString("MAIN_PDF_TITLE_BACKGROUND_COLOR") ='230,230,230';
			if (! empty(getDolGlobalString("MAIN_PDF_TITLE_BACKGROUND_COLOR"))) $pdf->Rect($this->marge_gauche, $tab_top, $this->page_largeur-$this->marge_droite-$this->marge_gauche, 5, 'F', null, explode(',', getDolGlobalString("MAIN_PDF_TITLE_BACKGROUND_COLOR")));
		}
	...
	protected function _pagehead(&$pdf, $object, $showHeadDetails, $outputlangs, $titlekey = "PdfCustomOrderTitle")
	{
		...
		// Translations
		$outputlangs->loadLangs(array('workshop@workshop', "main", "bills", "propal", "orders", "companies"));
		...
		$barcodetype=empty(getDolGlobalString("OPERATION_ORDER_BARCODE_TYPE"))?'C128':getDolGlobalString("OPERATION_ORDER_BARCODE_TYPE");
		$pdf->write1DBarcode('OR'.$object->ref, $barcodetype, $this->marge_gauche, $posy, 50, $barCodeHeight, '', $style);

		// Multicompagny
		if (!empty(isModEnabled("multicompany")) && $object->entity > 0) {
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 0);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("DatePlanned")." : ".dol_print_date($object->date_planned, "%d %b %Y", false, $outputlangs, true), '', 'R');
		...
		if ($showHeadDetails) {
			...
			$vehicule = new Vehicule($db);
			$res = $vehicule->fetch($object->fk_vehicule);
			if ($res<=0) {
				$vehicule->ref = $langs->trans('VehiculeNotFound');
			} else {
				$vehicule->getActivities($object->date_planned, $object->date_planned);
			}
			...
			$outInfos.= '<td class="titlefield"  >'.$langs->trans('Total HT').'</td>';
			$outInfos.= '<td ><strong>'.$object->total_ht.'</strong></td>';

			$outInfos.= '<td >&nbsp;</td>';

			$outInfos.= '<td class="titlefield"  >'.$outputlangs->transnoentities('PlannedDate').'</td>';
			$outInfos.= '<td ><strong>'.dol_print_date($object->date_planned, 'day', false, $outputlangs).'</strong></td>';
			$outInfos.= '</tr>';
			...
			$outVehicule.= '<td ></td>';
			$outVehicule.= '<td >&nbsp;</td>';
			$outVehicule.= '<td class="titlefield"  >'.$langs->trans($object->fields['km']['label']).'</td>';
			$outVehicule.= '<td ><strong>'.price($object->km).'</strong></td>';
			$outVehicule.= '</tr>';
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		return pdf_pagefoot($pdf, $outputlangs, 'ORDER_FREE_TEXT', null, $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext);
	}
	...
	/**
	 * Charge les lignes de l'OR dans $object->lines
	...
	function add_doc($object, $outputlangs, $pdf, $productref, $docname)
	{
		global $user, $langs;

		$langs->load('workshop@workshop');
		// Add Pages from models
		$infile= DOL_DATA_ROOT.'/produit/' .$productref . '/' . $docname .'.pdf';
