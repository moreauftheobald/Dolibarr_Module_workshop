...
if ($mode === 'dashboard') {
	...
	$wpLang = array(
		'Mechanics'     => $langs->transnoentities('WorkshopPlanningModeMecaniciens'),
		'NoMechanics'   => $langs->transnoentities('WorkshopPlanningNoMechanicsConfigured'),
		'Planned'       => $langs->transnoentities('WorkshopPlanningPlanned'),
		'Clocked'       => $langs->transnoentities('WorkshopPlanningClocked'),
		'Unassigned'    => $langs->transnoentities('WorkshopPlanningUnassigned'),
		'NoUnassigned'  => $langs->transnoentities('None'),
		'NewPointage'   => $langs->transnoentities('WorkshopNewPointage'),
		'EditPointage'  => $langs->transnoentities('WorkshopEditPointage'),
		'OnJob'         => $langs->transnoentities('WorkshopPointageOnJob'),
		'Improductive'  => $langs->transnoentities('WorkshopImproductive'),
		'OR'            => $langs->transnoentities('WorkshopOR'),
		'SearchOR'      => $langs->transnoentities('WorkshopSearchOR'),
		'Job'           => $langs->transnoentities('WorkshopJob'),
		'ImproCode'     => $langs->transnoentities('WorkshopImproCode'),
		'Start'         => $langs->transnoentities('WorkshopStartHour'),
		'End'           => $langs->transnoentities('WorkshopEndHour'),
		'EmptyOpen'     => $langs->transnoentities('WorkshopEmptyOpen'),
		'Note'          => $langs->transnoentities('Note'),
		'Delete'        => $langs->transnoentities('Delete'),
		'Save'          => $langs->transnoentities('Save'),
		'AssignJob'     => $langs->transnoentities('WorkshopAssignJob'),
		'DurationHours' => $langs->transnoentities('WorkshopDurationHours'),
		'Assign'        => $langs->transnoentities('WorkshopAssignAction'),
		'Pointage'      => $langs->transnoentities('Pointage'),
		'ConfirmDelete' => $langs->transnoentities('WorkshopConfirmDeletePointage'),
		'StartRequired' => $langs->transnoentities('WorkshopStartRequired'),
		'NetworkError'  => $langs->transnoentities('WorkshopNetworkError'),
		'Charge'        => $langs->transnoentities('WorkshopWeekCharge'),
		'NewOR'         => $langs->transnoentities('WorkshopNewOR'),
		'ThirdParty'    => $langs->transnoentities('ThirdParty'),
		'SearchThirdParty' => $langs->transnoentities('WorkshopSearchThirdParty'),
		'Vehicle'       => $langs->transnoentities('Vehicle'),
		'Description'   => $langs->transnoentities('Description'),
		'PlanNow'       => $langs->transnoentities('WorkshopPlanNow'),
		'Mechanic'      => $langs->transnoentities('WorkshopMechanic'),
		'CreateAndOpen' => $langs->transnoentities('WorkshopCreateAndOpenOR'),
	);
	...
	if ($user->hasRight('workshop', 'workshopplanning', 'write')) {
		...
		$statusPlanable=[];
		$status_ToPlann =  new WorkshopOperationOrderStatus($db);
		$resPlanable = $status_ToPlann->fetchAll(0,false,['planable'=>1,'status_type'=>WorkshopOperationOrderStatus::TYPE_OR_AND_JOB]);
		if (!is_array($resPlanable) && $resPlanable<0) {
			setEventMessages($status_ToPlann->error, $status_ToPlann->errors, 'errors');
		}

		if (is_array($resPlanable) && !empty($resPlanable) && $status_planned_id > 0) {

			$check_perms_toplan=true;
			foreach($resPlanable as $row){
				$status_perms_toplan =  new WorkshopOperationOrderStatus($db);
				$res = $status_perms_toplan->fetch($row->id);
				if ($res == null) {
					$check_perms_toplan=false;
				} else {
					$statusPlanable[]=$row->id;
					$check_perms_toplan = $check_perms_toplan && $status_perms_toplan->userCan($user, 'edit');
				}
			}
			$status_perms_planned =  new WorkshopOperationOrderStatus($db);
			$res = $status_perms_planned->fetch($status_planned_id);
			if ($res == null) {
				$check_perms_planned=true;
			} else {
				$check_perms_planned=$status_perms_planned->userCan($user,'changeToThisStatus');
			}
			if ($check_perms_toplan && $check_perms_planned) {
				if ($check_perms_toplan && $check_perms_planned) {
					...
					$sql_pl .= ' INNER JOIN ' . $db->prefix() . 'workshop_operationorder_jobs j ON o.rowid = j.fk_operationorder';
					...
					$sql_pl .= ' AND o.status IN (' . join(', ', $statusPlanable) . ')';
					$sql_pl .= ' AND j.status IN (' . join(', ', $statusPlanable) . ')';
