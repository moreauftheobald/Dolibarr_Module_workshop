...
/**
 *    	\file       vh_setup_type.php
 *      \ingroup    dolifleet
 *      \brief      Page to create/edit/view vehicule type
 */
...
$hookmanager->initHooks(array('vhsetuptype', 'globalcard')); // Note that conf->hooks_modules contains array
		...
		if (($action == 'confirmnew') || ($action == 'confirmedit' && !empty($rowid)) || ($action == 'confirmdelete' && !empty($rowid))) {
			...
			if (empty($error)) {
				if ($action == 'confirmnew') {
					$sql = "INSERT INTO " . MAIN_DB_PREFIX . "c_dolifleet_vehicule_type (entity,code, label, active, date_creation) VALUES (";
					$sql .= "'" . $conf->entity . "',";
					$sql .= "'" . $code . "',";
					$sql .= "'" . $label . "',";
					$sql .= "'" . $active . "',";
					...
					$sql = "UPDATE " . MAIN_DB_PREFIX . "c_dolifleet_vehicule_type SET ";
					$sql .= "code = '" . $code . "', ";
					$sql .= "label = '" . $label . "', ";
					$sql .= "active = '" . $active . "' ";
					$sql .= "WHERE rowid = " . $rowid;
				} elseif ($action == 'confirmdelete') {
					$sql = "DELETE FROM " . MAIN_DB_PREFIX . "c_dolifleet_vehicule_type WHERE rowid = " . $rowid;
				}
				$res = $db->query($sql);
				if (!$res) {
					$error++;
...
$sql .= "FROM ".MAIN_DB_PREFIX."c_dolifleet_vehicule_type as p ";
...
print dol_get_fiche_head($head, 'type', $langs->trans("DolifleetSetupType"), -1, "fontawesome_fa-tools");
// Part to show record

$formconfirm = '';
if ($action=='delete' && !empty($rowid)) {
	$formquestion[] = array('type'=>'hidden','name'=>'rowid','value'=>$rowid);
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('DeleteType'), $langs->trans('DeleteTypeQuestion'), 'confirmdelete', $formquestion, 'yes', 1);
} elseif ($action =='new') {
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('code'), 'name'=>'code','value'=> $code);
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('typelabel'), 'name'=>'label','value'=>$label);
	...
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('NewType'), '', 'confirmnew', $formquestion, 'yes', 1, 0, 700);
} elseif ($action =='edit' && !empty($rowid)) {
	$dataedit = $typearray[$rowid];
	$formquestion[] = array('type'=>'hidden','name'=>'rowid','value'=>$rowid);
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('code'), 'name'=>'code','value'=> $dataedit->code);
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('typelabel'), 'name'=>'label','value'=>$dataedit->label);
	$formquestion[] = array('type'=>'select','label'=>$langs->trans('active'), 'name'=>'active','values'=>array('0'=>'Non', '1'=>'Oui'), 'default'=>$dataedit->active);
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('EditType'), '', 'confirmedit', $formquestion, 'yes', 1, 0, 700);
}
...
// Print form confirm
print $formconfirm;
$actionpathnew = dol_buildpath('/workshop/vehicule/param/vh_setup_type.php', 2). '?action=new';
...
foreach ($typearray as $key=>$data) {
	...
	$actionpath = dol_buildpath('/workshop/vehicule/param/vh_setup_type.php', 2) . '?rowid=' . $data->id . '&action=';
