...
$hookmanager->initHooks(array('vhsetuppneu', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($resql) {
	...
	while ($i < ($limit ? min($limit, $num) : $num)) {
		...
		$pneu->id = $obj->rowid;
		$pneu->code = $obj->code;
		$pneu->label = $obj->label;
		$pneu->active = $obj->active;
		$pneuarray[$pneu->id] = $pneu;
...
$form = new Form($db);
$title = $langs->trans('WorkshopSetupPneu');
$help_url = '';
llxHeader('', $title, $help_url);

$head = workshopObjetsAnnexesPrepareHead();
print dol_get_fiche_head($head, 'pneu', $langs->trans("WorkshopAdminTabObjetsAnnexes"), -1, "fontawesome_fa-tools");
// Part to show record
...
// Print form confirm
print $formconfirm;
$actionpathnew = dol_buildpath('/workshop/vehicule/param/vh_setup_pneu.php', 2). '?action=new';
...
foreach ($pneuarray as $key=>$data) {
	...
	$actionpath = dol_buildpath('/workshop/vehicule/param/vh_setup_pneu.php', 2) . '?rowid=' . $data->id . '&action=';
