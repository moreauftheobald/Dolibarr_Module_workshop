...
$title = $langs->trans("Vehicule")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("Vehicule"));
}
$help_url = '';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-workshop page-card');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteVehicule'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	if ($action == 'xxx') {
		$text = $langs->trans('ConfirmActionVehicule', $object->ref);
...
//		if (!empty($object->lines) || ($object->status == $object::STATUS_DISACTIVATED && $permissiontoadd && $action != 'selectlines' && $action != 'editline')) {
...
//	}
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			if ($object->status == $object::STATUS_ACTIVATED) {
				print dolGetButtonAction('', $langs->trans('Disactivate'), 'default', $_SERVER["PHP_SELF"].'?id='.$object->id.'&action=confirm_setdraft&confirm=yes&token='.newToken(), '', $permissiontoadd);
			}
			...
			// Validate
			if ($object->status == $object::STATUS_DISACTIVATED) {
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('workshop:Vehicule', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('vehicule'));
