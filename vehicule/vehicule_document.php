...
/**
 *  \file       vehicule_document.php
 *  \ingroup    workshop
 *  \brief      Tab for documents linked to Vehicule
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->workshop->multidir_output[$object->entity ? $object->entity : $conf->entity]."/vehicule/".get_exdir(0, 0, 0, 1, $object);
}
...
// Header
// ------
$title = $langs->trans("Vehicule")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-workshop page-card_document');

// Show tabs
$head = vehiculePrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("Vehicule"), -1, $object->picto);
