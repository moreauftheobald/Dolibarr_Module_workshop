...
/**
 *   	\file       vehicule_list.php
 *		\ingroup    workshop
 *		\brief      List page for vehicule
 */
...
$title = $langs->trans("Vehicules");
//$help_url = "EN:Module_Vehicule|FR:Module_Vehicule_FR|ES:Módulo_Vehicule";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/workshop/vehicule/vehicule_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-workshop page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		...
		} elseif ((strpos($val['type'], 'integer:') === 0) || (strpos($val['type'], 'sellist:') === 0) || (strpos($val['type'], 'chkbxlst:') === 0)) {
