...
/**
 *    \file       vehicule/vehicule_list.php
 *    \ingroup    workshop
 *    \brief      Page to list vehicules
 */
...
$permissiontodelete = $user->hasRight('workshop', 'vehicule', 'delete');
...
// Special filter for dim_pneu (multi-select)
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/workshop/vehicule/vehicule_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
		...
		if (!empty($arrayfields['t.'.$key]['checked'])) {
			...
			if ($key == 'vin') {
				...
				foreach ($selected_vals as $sel) {
					...
					$dimLabel = getDictionaryValue('workshop_vehicule_c_vehicule_dimpneu', 'label', $sel);
