...
/**
 *  \file       vehicule_note.php
 *  \ingroup    workshop
 *  \brief      Tab for notes on Vehicule
 */
...
$langs->loadLangs(array("workshop@workshop", "companies"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->workshop->multidir_output[empty($object->entity) ? $conf->entity : $object->entity]."/".$object->id;
}
...
$title = $langs->trans('Vehicule').' - '.$langs->trans("Notes");
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("Vehicule"), -1, $object->picto);
