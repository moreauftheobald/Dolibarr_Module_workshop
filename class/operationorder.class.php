...
class Operationorder extends CommonObject
{
	...
	public $fields = array(
		...
		'date_start'     => array('type' => 'date',          'label' => 'DateStart',      'enabled' => 1, 'position' => 80,  'notnull' => 0, 'visible' => 1),
		'date_end'       => array('type' => 'date',          'label' => 'DateEnd',        'enabled' => 1, 'position' => 90,  'notnull' => 0, 'visible' => 1),
	...
	public function getBanner($action,$permissiontoadd) {
		...
		$orTagList = $orTagObj->getTagsForOR($this->id);
		...
		if ($action == 'editfk_vehicule' && $permissiontoadd) {
			...
			if ($this->fk_vehicule > 0) {
				...
				if ($vehicule->fetch($this->fk_vehicule) > 0) {
