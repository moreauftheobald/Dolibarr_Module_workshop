...
/**
 * \file        class/vehiculecontracttype.class.php
 * \ingroup     workshop
 * \brief       This file is a CRUD class file for VehiculeContractType (Create/Read/Update/Delete)
 */
...
/**
 * Class for VehiculeContractType
 */
class VehiculeContractType extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'vehiculecontracttype';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'workshop_vehicule_c_contract_type';

	/**
	 * @var string 	If permission must be checkec with hasRight('workshop', 'read') and not hasright('mymodyle', 'vehiculecontracttype', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'workshop';

	/**
	 * @var string 	String with name of icon for vehiculecontracttype. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'vehiculecontracttype@workshop' if picto is file 'img/object_vehiculecontracttype.png'.
	 */
	...
	//  */
	// public $table_element_line = 'workshop_vehiculecontracttypeline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_vehiculecontracttype';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'VehiculeContractTypeline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'VehiculeContractType', 'fk_element'=>'fk_vehiculecontracttype'));
	...
	//  */
	// protected $childtablesoncascade = array('workshop_vehiculecontracttypedet');

	// /**
	//  * @var VehiculeContractTypeLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('workshop', 'vehiculecontracttype', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('workshop', 'vehiculecontracttype', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('workshop', 'vehiculecontracttype_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'vehiculecontracttype/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'vehiculecontracttype/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'vehiculecontracttype/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'vehiculecontracttype/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->workshop->dir_output.'/vehiculecontracttype/'.$oldref;
				$dirdest = $conf->workshop->dir_output.'/vehiculecontracttype/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->workshop->dir_output.'/vehiculecontracttype/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/workshop/vehiculecontracttype_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowVehiculeContractType");
	...
	public function getLinesArray()
	{
		...
		$objectline = new VehiculeContractTypeLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_vehiculecontracttype:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('WORKSHOP_MYOBJECT_ADDON')) {
			$conf->global->WORKSHOP_MYOBJECT_ADDON = 'mod_vehiculecontracttype_standard';
		}
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_vehiculecontracttype';
...
/**
 * Class VehiculeContractTypeLine. You can also remove this and generate a CRUD class for lines objects.
 */
class VehiculeContractTypeLine extends CommonObjectLine
{
	// To complete with content of an object VehiculeContractTypeLine
	// We should have a field rowid, fk_vehiculecontracttype and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'vehiculecontracttype'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_vehiculecontracttype'
