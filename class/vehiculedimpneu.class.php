...
/**
 * \file        class/vehiculedimpneu.class.php
 * \ingroup     workshop
 * \brief       This file is a CRUD class file for VehiculeDimPneu (Create/Read/Update/Delete)
 */
...
/**
 * Class for VehiculeDimPneu
 */
class VehiculeDimPneu extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'vehiculedimpneu';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'workshop_vehiculedimpneu';
	...
	//  */
	// public $table_element_line = 'workshop_vehiculedimpneuline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_vehiculedimpneu';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'VehiculeDimPneuline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'VehiculeDimPneu', 'fk_element'=>'fk_vehiculedimpneu'));
	...
	//  */
	// protected $childtablesoncascade = array('workshop_vehiculedimpneudet');

	// /**
	//  * @var VehiculeDimPneuLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('workshop', 'vehiculedimpneu', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('workshop', 'vehiculedimpneu', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('workshop', 'vehiculedimpneu_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'vehiculedimpneu/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'vehiculedimpneu/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'vehiculedimpneu/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'vehiculedimpneu/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->workshop->dir_output.'/vehiculedimpneu/'.$oldref;
				$dirdest = $conf->workshop->dir_output.'/vehiculedimpneu/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->workshop->dir_output.'/vehiculedimpneu/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowVehiculeDimPneu")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("VehiculeDimPneu").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/workshop/vehiculedimpneu_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowVehiculeDimPneu");
	...
	public function getLinesArray()
	{
		...
		$objectline = new VehiculeDimPneuLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_vehiculedimpneu:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('WORKSHOP_MYOBJECT_ADDON')) {
			$conf->global->WORKSHOP_MYOBJECT_ADDON = 'mod_vehiculedimpneu_standard';
		}
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_vehiculedimpneu';
...
/**
 * Class VehiculeDimPneuLine. You can also remove this and generate a CRUD class for lines objects.
 */
class VehiculeDimPneuLine extends CommonObjectLine
{
	// To complete with content of an object VehiculeDimPneuLine
	// We should have a field rowid, fk_vehiculedimpneu and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'vehiculedimpneu'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_vehiculedimpneu'
