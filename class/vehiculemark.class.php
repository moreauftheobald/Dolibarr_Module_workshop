 ...
 * \ingroup     workshop
 * \brief       This file is a CRUD class file for VehiculeMark (Create/Read/Update/Delete)
 */
...
/**
 * Class for VehiculeMark
 */
class VehiculeMark extends CommonObject
{
	/**
	...
	/**
	 * @var string 	If permission must be checkec with hasRight('workshop', 'read') and not hasright('mymodyle', 'vehiculemark', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'workshop';

	/**
	 * @var string 	String with name of icon for vehiculemark. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'vehiculemark@workshop' if picto is file 'img/object_vehiculemark.png'.
	 */
	...
	//  */
	// public $table_element_line = 'workshop_vehiculemarkline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_vehiculemark';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'VehiculeMarkline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'VehiculeMark', 'fk_element'=>'fk_vehiculemark'));
	...
	//  */
	// protected $childtablesoncascade = array('workshop_vehiculemarkdet');

	// /**
	//  * @var VehiculeMarkLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('workshop', 'vehiculemark', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('workshop', 'vehiculemark', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('workshop', 'vehiculemark_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'vehiculemark/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'vehiculemark/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'vehiculemark/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'vehiculemark/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->workshop->dir_output.'/vehiculemark/'.$oldref;
				$dirdest = $conf->workshop->dir_output.'/vehiculemark/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->workshop->dir_output.'/vehiculemark/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowVehiculeMark")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("VehiculeMark").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/workshop/vehiculemark_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowVehiculeMark");
	...
	public function getLinesArray()
	{
		...
		$objectline = new VehiculeMarkLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_vehiculemark:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('WORKSHOP_MYOBJECT_ADDON')) {
			$conf->global->WORKSHOP_MYOBJECT_ADDON = 'mod_vehiculemark_standard';
		}
...
/**
 * Class VehiculeMarkLine. You can also remove this and generate a CRUD class for lines objects.
 */
class VehiculeMarkLine extends CommonObjectLine
{
	// To complete with content of an object VehiculeMarkLine
	// We should have a field rowid, fk_vehiculemark and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'vehiculemark'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_vehiculemark'
