...
// Permissions
$permissiontoread = $user->hasRight('dolifleet', 'read');
$permissiontoadd = $user->hasRight('dolifleet', 'write');
$permissiontodelete = $user->hasRight('dolifleet', 'delete');
...
$sql .= ' WHERE t.entity IN ('.getEntity('dolifleet', 1).')';
...
// Special filters preserved from original code
if (GETPOSTISSET('dim_pneu')) {
	$dimPneuArray = GETPOST('dim_pneu', 'array');
	if (!empty($dimPneuArray)) {
		$cleanIds = array_map('intval', $dimPneuArray);
		$sql .= ' AND t.dim_pneu IN ('.implode(',', $cleanIds).')';
	}
}
if (GETPOSTISSET('atelier') && GETPOST('atelier', 'int') > 0) {
	$sql .= ' AND t.atelier = '.((int) GETPOST('atelier', 'int'));
}
...
llxHeader('', $title, $help_url, '', 0, 0, array(), array(), '', 'mod-dolifleet page-list bodyforlist');
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/dolifleet/vehicule_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$topicmail = "SendVehiculeRef";
$modelmail = "vehicule";
$objecttmp = new Vehicule($db);
...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		...
		if (!empty($val['arrayofkeyval']) && is_array($val['arrayofkeyval'])) {
			...
			print '</div>';
		} elseif (strpos($val['type'], 'chkbxlst:') === 0) {
			// For chkbxlst type, use showInputField
			print $object->showInputField($val, $key, (isset($search[$key]) ? $search[$key] : ''), '', '', 'search_', $cssforfield.' maxwidth250', 1);
		} else {
...
while ($i < $imaxinloop) {
	...
	foreach ($object->fields as $key => $val) {
		...
		if (!empty($arrayfields['t.'.$key]['checked'])) {
			...
			if (preg_match('/tdoverflow/', $cssforfield) && !in_array($val['type'], array('ip', 'url')) && !is_numeric($object->$key) && !in_array($key, array('vin'))) {
			...
			} elseif ($key == 'dim_pneu' && !empty($object->dim_pneu)) {
				// Special rendering for dim_pneu (chkbxlst displayed as badges)
				$output = '<div class="select2-container-multi-dolibarr" style="width: 90%;"><ul class="select2-choices-dolibarr">';
				$selected_vals = explode(',', $object->dim_pneu);
				foreach ($selected_vals as $sel) {
					if (empty($sel)) {
						continue;
					}
					$dimLabel = getDictionaryValue('c_dolifleet_vehicule_dimpneu', 'label', $sel);
					$output .= '<li class="select2-search-choice-dolibarr noborderoncategories" style="background: #bbb">'.$dimLabel.'</li>';
				}
				$output .= '</ul></div>';
				print $output;
			} else {
