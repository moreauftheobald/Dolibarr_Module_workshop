...
$langs->loadLangs(array('products', 'companies','operationorder'));
...
$hookmanager->initHooks(array('productstatsoperationoder'));
...
if (!$sortfield) $sortfield = "o.date_creation";
...
if ($id > 0 || !empty($ref)) {
	...
	if ($result > 0) {
		...
		if ($user->hasRight("operationorder", "read")) {
			$sql = "SELECT DISTINCT s.nom as name, s.rowid as socid, o.rowid as orid, o.ref, o.total_ht as amount,";
			$sql .= " o.ref_client,";
			$sql .= "o.date_creation, o.status as statut, d.rowid, d.qty";
			if (!$user->hasRight("societe", "client", "voir") && !$socid)
				$sql .= ", sc.fk_soc, sc.fk_user ";
			$sql .= ", v.immatriculation";
			$sql .= " FROM ".$db->prefix()."societe as s";
			$sql .= ",".$db->prefix()."operationorder as o";
			$sql .= ", ".$db->prefix()."operationorderdet as d";
			if (!$user->hasRight("societe", "client", "voir") && !$socid)
				$sql .= ", ".$db->prefix()."societe_commerciaux as sc";
			$sql .= ", ".$db->prefix()."dolifleet_vehicule as v";
			$sql .= " WHERE o.fk_soc = s.rowid";
			$sql .= " AND o.entity IN (".getEntity('operationorder').")";
			$sql .= " AND d.fk_operation_order = o.rowid";
			$sql .= " AND v.rowid = o.fk_vehicule";
			$sql .= " AND d.fk_product =".$product->id;
			if (!empty($search_month))
				$sql .= " AND MONTH(o.date_creation) IN (".$search_month.")";
			if (!empty($search_year))
				$sql .= " AND YEAR(o.date_creation) IN (".$search_year.")";
			if (!empty($search_immat))
				$sql .= " AND v.immatriculation LIKE '%" . $search_immat . "%'" ;
			if (!$user->hasRight("societe", "client", "voir") && !$socid)
				$sql .= " AND s.rowid = sc.fk_soc AND sc.fk_user = ".$user->id;
			if ($socid)
				$sql .= " AND o.fk_soc = ".$socid;
			...
			if ($result) {
				...
				print_barre_liste($langs->trans("ShowOperationOrder"), $page, $_SERVER["PHP_SELF"], $option, $sortfield, $sortorder, '', $num, $totalofrecords, '', 0, '', '', $limit, 0, 0, 1);
				...
				print $langs->trans('Vehicule').':<input class="flat" type="text" size="8" name="search_immat" value="'.$search_immat.'"> ';
				...
				print_liste_field_titre("Vehicule", $_SERVER["PHP_SELF"], "v.immatriculation", "", $option, '', $sortfield, $sortorder);
				print_liste_field_titre("DateCreationOperationOrder", $_SERVER["PHP_SELF"], "p.datep", "", $option, 'align="center"', $sortfield, $sortorder);
				...
				if ($num > 0) {
					while ($i < min($num, $limit)) {
						...
						print "</td>\n";
						print '<td>'.$OR->thirdparty->getNomUrl(1).'</td>';
						print '<td>'. $objp->immatriculation .'</td>';
						print '<td class="center">';
						print dol_print_date($db->jdate($objp->date_creation), 'daytext')."</td>";
						print "<td align=\"center\">".$objp->qty."</td>\n";
						print '<td align="right">'.price($objp->amount).'</td>'."\n";
						print '<td align="right">'.$OR->LibStatut($objp->statut, 5).'</td>';
						print "</tr>\n";
						$i++;
					}
				}
