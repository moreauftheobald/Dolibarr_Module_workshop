...
//rights
$usercanmodifyuserplanning = $user->hasRight("operationorder", "userplanning", "write");
$usercanmodifygroupplanning = $user->hasRight("operationorder", "usergroupplanning", "write");
...
if ($objecttype == 'user') {
	...
	print dol_get_fiche_head($head, 'userplanning', $title, -1, 'user');
	dol_banner_tab($object, 'id', $linkback, 0);

	$userCanModify = $user->hasRight("operationorder", "userplanning", "write");
} elseif ($objecttype == 'usergroup') {
	$head = group_prepare_head($object);
	$title = $langs->trans("Group");
	print dol_get_fiche_head($head, 'usergroupplanning', $title, -1, 'group');
	dol_banner_tab($object, 'id', $linkback, 0);

	$userCanModify = $user->hasRight("operationorder", "usergroupplanning", "write");
}
