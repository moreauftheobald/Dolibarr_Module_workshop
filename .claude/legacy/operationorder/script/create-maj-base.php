...
$sqlAgenda.= ' FROM '.$db->prefix().'c_action_trigger';
$sqlAgenda.= $db->plimit(1);
$agendaTriggerRank=0;
...
foreach ($TAgendaTriggers as $agendaTrigger) {
	...
	$sqlAgenda = 'SELECT COUNT(*) as alreadyExists  FROM '.$db->prefix().'c_action_trigger as a WHERE  a.code = \''.$db->escape($agendaTrigger['code']).'\' LIMIT 1';
	$resql=$db->query($sqlAgenda);
	if ($resql) {
		$obj = $db->fetch_object($resql);

		if (empty($obj->alreadyExists)) {
			$sqlAgenda = 'insert into '.$db->prefix().'c_action_trigger (code,label,description,elementtype,rang)';
			...
			$sqlAgenda = 'UPDATE '.$db->prefix().'c_action_trigger SET ';
			$Tfields = array();
