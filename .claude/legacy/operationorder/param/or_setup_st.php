...
$doc_obl_sel =  GETPOSTISSET('doc_obl_sel') ? GETPOST('doc_obl_sel', 'alpha') : 0;
		...
		if (empty($error)) {
			if ($action == 'confirmnew') {
				...
				if (empty($error)) {
					...
					$product->array_options['oorder_available_for_supplier_order'] = 1;
...
$sql  = "SELECT p.rowid as rowid, p.ref as ref, p.label as label, p.tosell as tosell, p.tobuy as tobuy, ef.doc_obl, p.price as price, p.price_min as price_min, p.price_base_type as price_base_type ";
$sql .= "FROM ".$db->prefix()."product as p ";
$sql .= "INNER JOIN ".$db->prefix()."product_extrafields as ef ON p.rowid = ef.fk_object ";
$sql .= "WHERE p.entity IN (".getEntity('product').") AND p.fk_product_type=1 AND ef.oorder_available_for_supplier_order =1 ";
...
if ($resql) {
	...
	while ($i < ($limit ? min($limit, $num) : $num)) {
		...
		$st->ref = $obj->ref;
		$st->label = $obj->label;
		$st->tosell = $obj->tosell;
		$st->tobuy = $obj->tobuy;
		$st->doc_obl = $obj->doc_obl;
		$st->price = $obj->price;
		$st->price_min = $obj->price_min;
		$st->price_base_type = $obj->price_base_type;
		$starray[$st->id] = $st;
...
$title = $langs->trans('ParamOrSetupSt');
$help_url = '';
llxHeader('', $title, $help_url);

$head = OrSetupPrepareHead();
print dol_get_fiche_head($head, 'st', $langs->trans("ParamOrSetupSt"), -1, "fontawesome_fa-tools");
// Part to show record

$formconfirm = '';
if ($action=='delete' && !empty($rowid)) {
	$formquestion[] = array('type'=>'hidden','name'=>'rowid','value'=>$rowid);
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('DeleteSt'), $langs->trans('DeleteStQuestion'), 'confirmdelete', $formquestion, 'yes', 1);
} elseif ($action =='new') {
	...
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('stlabel'), 'name'=>'label','value'=>$label);
	...
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('NewSt'), '', 'confirmnew', $formquestion, 'yes', 1, 0, 700);
} elseif ($action =='edit' && !empty($rowid)) {
	$dataedit = $starray[$rowid];
	$product = new Product($db);
	$product->fetch($rowid);
	$upload_dir = $conf->user->multidir_temp[$conf->entity];
	...
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('stlabel'), 'name'=>'label','value'=>$dataedit->label);
	...
	$formquestion[] = array('type'=>'other','label'=>$langs->trans('doc_obligatoire'), 'value'=>$out);
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('EditSt'), '', 'confirmedit', $formquestion, 'yes', 1, 0, 700);
}
...
// Print form confirm
print $formconfirm;
$actionpathnew = dol_buildpath('/operationorder/param/or_setup_st.php', 3). '?action=new';
...
print '<th class="liste_titre">' . $langs->trans("stlabel") . '</th>';
...
foreach ($starray as $key=>$data) {
	...
	print '</td>';
	$actionpath = dol_buildpath('/operationorder/param/or_setup_st.php', 3) . '?rowid=' . $data->id . '&action=';
