...
$price_min = GETPOST('price_min', 'alpha');
		...
		if (empty($error)) {
			if ($action == 'confirmnew') {
				...
				if (empty($error)) {
					...
					$product->tva_tx = 20;
					$product->accountancy_code_buy = getDolGlobalString("ACCOUNTING_SERVICE_SOLD_ACCOUNT");
					$product->accountancy_code_buy_export = getDolGlobalString("ACCOUNTING_SERVICE_SOLD_INTRA_ACCOUNT");
					$product->accountancy_code_buy_intra = getDolGlobalString("ACCOUNTING_SERVICE_SOLD_EXPORT_ACCOUNT");
					$product->accountancy_code_sell = getDolGlobalString("ACCOUNTING_SERVICE_BUY_ACCOUNT");
					$product->accountancy_code_sell_export = getDolGlobalString("ACCOUNTING_SERVICE_BUY_INTRA_ACCOUNT");
					$product->accountancy_code_sell_intra = getDolGlobalString("ACCOUNTING_SERVICE_BUY_EXPORT_ACCOUNT");
					if (!empty($file_uplaoded)) {
						$product->array_options['options_doc_obl'] = basename($file_uplaoded, ".pdf");
					}
					$product->array_options['options_or_is_job'] = 0;
				...
				if ($resfetch) {
					...
					if ($product->price != $price ||$product->price_min != $price_min) {
						$product->updatePrice($price, 'HT', $user, 20, $price_min);
					}
...
$sql  = "SELECT p.rowid as rowid, p.ref as ref, p.label as label, p.tosell as tosell, p.tobuy as tobuy, ";
$sql .= "ef.doc_obl, p.price as price, p.price_min as price_min, p.price_base_type as price_base_type ";
$sql .= "FROM ".$db->prefix()."product as p ";
$sql .= "INNER JOIN ".$db->prefix()."product_extrafields as ef ON p.rowid = ef.fk_object ";
$sql .= "WHERE p.entity IN (".getEntity('product').") AND p.fk_product_type=1 ";
$sql .= "AND (ef.or_scan = 0 OR ef.or_scan IS NULL) ";
$sql .= "AND (ef.or_is_job = 0 OR ef.or_is_job IS NULL) ";
$sql .= "AND (ef.oorder_available_for_supplier_order = 0 OR ef.oorder_available_for_supplier_order IS NULL)";
...
if ($resql) {
	...
	while ($i < ($limit ? min($limit, $num) : $num)) {
		...
		$serv->id = $obj->rowid;
		$serv->ref = $obj->ref;
		$serv->label = $obj->label;
		$serv->tosell = $obj->tosell;
		$serv->tobuy = $obj->tobuy;
		$serv->doc_obl = $obj->doc_obl;
		$serv->price = $obj->price;
		$serv->price_min = $obj->price_min;
		$serv->price_base_type = $obj->price_base_type;
		$servarray[$serv->id] = $serv;
...
$title = $langs->trans('ParamOrSetupServ');
$help_url = '';
llxHeader('', $title, $help_url);

$head = OrSetupPrepareHead();
print dol_get_fiche_head($head, 'sv', $langs->trans("ParamOrSetupServ"), -1, "fontawesome_fa-tools");
// Part to show record

$formconfirm = '';
if ($action=='delete' && !empty($rowid)) {
	$formquestion[] = array('type'=>'hidden','name'=>'rowid','value'=>$rowid);
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('DeleteServ'), $langs->trans('DeleteServQuestion'), 'confirmdelete', $formquestion, 'yes', 1);
} elseif ($action =='new') {
	...
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('servlabel'), 'name'=>'label','value'=>$label);
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('price'), 'name'=>'price','value'=>$price);
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('price_min'), 'name'=>'price_min','value'=>$price_min);
	...
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('NewServ'), '', 'confirmnew', $formquestion, 'yes', 1, 0, 700);
} elseif ($action =='edit' && !empty($rowid)) {
	$dataedit = $servarray[$rowid];
	$product = new Product($db);
	$product->fetch($rowid);
	$upload_dir = $conf->user->multidir_temp[$conf->entity];
	...
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('servlabel'), 'name'=>'label','value'=>$dataedit->label);
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('price'), 'name'=>'price','value'=>$dataedit->price);
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('price_min'), 'name'=>'price_min','value'=>$dataedit->price_min);
	...
	$formquestion[] = array('type'=>'other','label'=>$langs->trans('doc_obligatoire'), 'value'=>$out);
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('EditServ'), '', 'confirmedit', $formquestion, 'yes', 1, 0, 700);
}
...
// Print form confirm
print $formconfirm;
$actionpathnew = dol_buildpath('/operationorder/param/or_setup_serv.php', 3). '?action=new';
...
print '<th class="liste_titre">' . $langs->trans("servlabel") . '</th>';
print '<th class="liste_titre">' . $langs->trans("price") . '</th>';
print '<th class="liste_titre">' . $langs->trans("price_min") . '</th>';
...
print '</tr>';
foreach ($servarray as $key=>$data) {
	$product_static->id = $data->id;
	$product_static->ref = $data->ref;
	print '<tr class="oddeven">';
	print '<td>' . $product_static->getNomUrl(1) . '</td>';
	print '<td>' . $data->label . '</td>';
	print '<td>' . price($data->price) . ' ' . $data->price_base_type . '</td>';
	print '<td>' . price($data->price_min) . ' ' . $data->price_base_type . '</td>';
	...
	print '</td>';
	$actionpath = dol_buildpath('/operationorder/param/or_setup_serv.php', 3) . '?rowid=' . $data->id . '&action=';
