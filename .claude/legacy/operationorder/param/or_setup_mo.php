		...
		if (empty($error)) {
			if ($action == 'confirmnew') {
				...
				if (empty($error)) {
					...
					$product->array_options['options_or_scan'] = 1;
...
$sql .= "WHERE p.entity IN (".getEntity('product').") AND p.fk_product_type=1 AND ef.or_scan =1 ";
...
if ($resql) {
	...
	while ($i < ($limit ? min($limit, $num) : $num)) {
		...
		$mo->ref = $obj->ref;
		$mo->label = $obj->label;
		$mo->tosell = $obj->tosell;
		$mo->tobuy = $obj->tobuy;
		$mo->doc_obl = $obj->doc_obl;
		$mo->price = $obj->price;
		$mo->price_min = $obj->price_min;
		$mo->price_base_type = $obj->price_base_type;
		$moarray[$mo->id] = $mo;
...
$title = $langs->trans('ParamOrSetupMo');
$help_url = '';
llxHeader('', $title, $help_url);

$head = OrSetupPrepareHead();
print dol_get_fiche_head($head, 'mo', $langs->trans("ParamOrSetupMo"), -1, "fontawesome_fa-tools");
// Part to show record

$formconfirm = '';
if ($action=='delete' && !empty($rowid)) {
	$formquestion[] = array('type'=>'hidden','name'=>'rowid','value'=>$rowid);
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('DeleteMo'), $langs->trans('DeleteMoQuestion'), 'confirmdelete', $formquestion, 'yes', 1);
} elseif ($action =='new') {
	...
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('molabel'), 'name'=>'label','value'=>$label);
	...
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('NewMo'), '', 'confirmnew', $formquestion, 'yes', 1, 0, 700);
} elseif ($action =='edit' && !empty($rowid)) {
	$dataedit = $moarray[$rowid];
	$product = new Product($db);
	$product->fetch($rowid);
	$upload_dir = $conf->user->multidir_temp[$conf->entity];
	...
	$formquestion[] = array('type'=>'text','label'=>$langs->trans('molabel'), 'name'=>'label','value'=>$dataedit->label);
	...
	$formquestion[] = array('type'=>'other','label'=>$langs->trans('doc_obligatoire'), 'value'=>$out);
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('EditMo'), '', 'confirmedit', $formquestion, 'yes', 1, 0, 700);
}
...
// Print form confirm
print $formconfirm;
$actionpathnew = dol_buildpath('/operationorder/param/or_setup_mo.php', 3). '?action=new';
...
print '<th class="liste_titre">' . $langs->trans("molabel") . '</th>';
...
foreach ($moarray as $key=>$data) {
	...
	print '</td>';
	$actionpath = dol_buildpath('/operationorder/param/or_setup_mo.php', 3) . '?rowid=' . $data->id . '&action=';
