...
if (!$user->hasRight("operationorder", "status", "read")) accessforbidden();
...
$permissiondellink = $user->hasRight("operationorder", "status", "write");	// Used by the include of actions_dellink.inc.php
...
if (empty($reshook)) {
	...
	switch ($action) {
		...
		case 'update':
			...
			$object->update_vehicule_info = GETPOST('update_vehicule_info', 'int'); // Lorsque la checkbox est décochée, le $_REQUEST ne contient pas l'élément ce qui fait la value n'est pas setté
			$object->require_conf = GETPOST('require_conf', 'int'); // Lorsque la checkbox est décochée, le $_REQUEST ne contient pas l'élément ce qui fait la value n'est pas setté
		...
		case 'modif':
		case 'activate':
			if ($user->hasRight("operationorder", "status", "write")) $object->setActive($user);

			break;
		case 'disable':
			if ($user->hasRight("operationorder", "status", "write")) $object->setDisabled($user);

			break;
		case 'confirm_cancel':
			if ($user->hasRight("operationorder", "status", "write")) $object->setCancel($user);

			header('Location: '.dol_buildpath('/operationorder/operationorderstatus_card.php', 1).'?id='.$object->id);
			exit;

		case 'confirm_delete':
			if ($user->hasRight("operationorder", "delete")) $object->delete($user);
...
if ($action == 'create') {
	...
	if (empty($object->id)) {
		...
		if (!empty($object->id) && $action === 'edit') {
			...
			print dol_get_fiche_head($head, 'card', $langs->trans('OperationOrderStatus'), 0, $picto);
			...
			$head = operationOrderStatusPrepareHead($object);
			$picto = 'operationOrderStatus@operationorder';
			print dol_get_fiche_head($head, 'card', $langs->trans('OperationOrderStatus'), -1, $picto);
			...
			if (empty($reshook)) {
				...
				print dolGetButtonAction($langs->trans("OperationOrderStatusEdit"), '', 'default', $actionUrl . 'edit', '', $user->hasRight("operationorder", "status", "write"));


				if ($object->status != OperationOrderStatus::STATUS_ACTIVE) {
					print dolGetButtonAction($langs->trans("OperationOrderStatusActivate"), '', 'default', $actionUrl . 'activate', '', $user->hasRight("operationorder", "status", "write"));
				}
				if ($object->status != OperationOrderStatus::STATUS_DISABLED) {
					print dolGetButtonAction($langs->trans("OperationOrderStatusDisable"), '', 'default', $actionUrl . 'disable', '', $user->hasRight("operationorder", "status", "write"));
				}


				print dolGetButtonAction($langs->trans("OperationOrderStatusDelete"), '', 'danger', $actionUrl . 'delete', '', $user->hasRight("operationorder", "delete"));
			}
			print '</div>'."\n";
