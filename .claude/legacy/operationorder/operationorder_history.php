...
if ($operationOrder->id > 0) {
	...
	print dol_get_fiche_head($head, 'history', $langs->trans('OperationOrder'), -1, $picto);
...
$sqlSelect = 'SELECT '.$fieldList;
$sql = '';
// Add fields from hooks
$parameters=array('sql' => $sql, 'sqlSelect' => $sqlSelect);
$reshook=$hookmanager->executeHooks('printFieldListSelect', $parameters, $object);    // Note that $action and $object may have been modified by hook
$sql.=$hookmanager->resPrint;

$sql.= ' FROM '.$db->prefix().'operationorderhistory t ';
$sql.= ' LEFT JOIN '.$db->prefix().'operationorder oo ON (t.fk_operationorder = oo.rowid) ';
$sql.= ' LEFT JOIN '.$db->prefix().'user u ON (t.fk_user_creat = u.rowid) ';
$sql.= ' LEFT JOIN '.$db->prefix().'operationorderdet ood ON (t.fk_operationorderdet = ood.rowid) ';
$sql.= ' LEFT JOIN '.$db->prefix().'product p ON (ood.fk_product = p.rowid) ';
...
$listViewConfig = array(
	...
	'search'			  => array(
		'date_creation'		   => array('search_type' => 'calendars', 'allow_is_null' => false,'table' => 't', 'field' => array('t.date_creation')),
		...
		'fk_user_creat'		   => array('search_type' => true, 'table' => 'u', 'field' => array('u.lastname', 'u.firstname')), // input text de recherche sur plusieurs champs,
...
$sqlCountTotal = 'SELECT Count(*) as count ' . $sql;
$resql = $db->query($sqlCountTotal);
if (!$resql) {
	setEventMessages($db->lasterror(), [], 'errors');
}

$obj = $db->fetch_object($resql);
$r->totalRow = $obj->count;

$conf->global->MAIN_DISABLE_FULL_SCANLIST = 1;
echo $r->render($sqlSelect . $sql, $listViewConfig);
$conf->global->MAIN_DISABLE_FULL_SCANLIST = 0;
...
function _getProdNomURL($fk_operationorderdet)
{
	...
	if (!empty($fk_operationorderdet)) {
		$ooDet = new operationorderLine($db);
