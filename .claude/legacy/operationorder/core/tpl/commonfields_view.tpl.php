...
foreach ($object->fields as $key => $val) {
	...
	print '</tr>';
	if ($key == 'fk_vehicule') {
		$fieldsmore = array('date_immat', 'fk_vehicule_type', 'fk_vehicule_mark', 'fk_contract_type_full');
		foreach ($fieldsmore as $fieldtoadd) {
			print '<tr class="trcommonfield_' . $fieldtoadd . '">';
			print '<td class="titlefield"  >';
			print $langs->trans($vehicule->fields[$fieldtoadd]['label']);
			print '</td>';
			print '<td >' . $vehicule->showOutputField($fieldtoadd) . '</td>';
			print '</tr>';
		}
	}
}
...
foreach ($object->fields as $key => $val) {
	...
	if ($key == 'fk_vehicule') {
		$fieldsmore = array('operations', 'date_immat', 'fk_vehicule_type', 'fk_vehicule_mark', 'fk_contract_type_full', 'linkedvh');
		foreach ($fieldsmore as $fieldtoadd) {
			print '<tr class="trcommonfield_' . $fieldtoadd . '">';
			print '<td class="titlefield"  >';
			if ($fieldtoadd == 'fk_contract_type_full') {
				print $langs->trans($vehicule->fields['fk_contract_type']['label']);
			} elseif ($fieldtoadd == 'operations') {
				print $langs->trans('operations');
			} elseif ($fieldtoadd == 'linkedvh') {
				print $langs->trans('linkedvh');
			} else {
				print $langs->trans($vehicule->fields[$fieldtoadd]['label']);
			}
			print '</td>';
			print '<td >' . $vehicule->showOutputFieldQuick($fieldtoadd) . '</td>';
			print '</tr>';
		}
	}
	print '</td>';
