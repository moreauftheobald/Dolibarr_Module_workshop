...
$objectline = new operationorderLine($db);
...
if ($nolinesbefore) {
	...
	<tr class="liste_titre<?php echo (($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_add_') ?> nodrag nodrop">
		<?php if (!empty(getDolGlobalString("MAIN_VIEW_LINE_NUMBER"))) { ?>
...
<tr class="pair nodrag nodrop nohoverpair<?php echo ($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_create'; ?>">
	...
	<td class="nobottom linecoldescription minwidth500imp">
		...
		<?php
		$freelines = false;
		if (empty(getDolGlobalString("MAIN_DISABLE_FREE_LINES"))) {
			$freelines = true;
			...
			if ($object->element == 'contrat') {
				if (empty($conf->product->enabled) && empty($conf->service->enabled) && empty(getDolGlobalString("CONTRACT_SUPPORT_PRODUCTS"))) $forceall = -1; // With contract, by default, no choice at all, except if CONTRACT_SUPPORT_PRODUCTS is set
				elseif (empty(getDolGlobalString("CONTRACT_SUPPORT_PRODUCTS"))) $forceall = 3;
			}
			// Free line
		...
		if (!empty($conf->product->enabled) || !empty($conf->service->enabled)) {
			...
			if (empty($senderissupplier)) {
				...
				elseif ((empty($conf->product->enabled) && !empty($conf->service->enabled)) || ($object->element == 'contrat' && empty(getDolGlobalString("CONTRACT_SUPPORT_PRODUCTS")))) echo $langs->trans('PredefinedServicesToSell');
			...
			echo ' ';
			$filtertype = '';
			if (!empty($object->element) && $object->element == 'contrat' && empty(getDolGlobalString("CONTRACT_SUPPORT_PRODUCTS"))) $filtertype = '1';
			if (empty($senderissupplier)) {
				$statustoshow = 1;
				if (!empty(getDolGlobalString("ENTREPOT_EXTRA_STATUS"))) {
				...
				if (!empty(getDolGlobalString("MAIN_AUTO_OPEN_SELECT2_ON_FOCUS_FOR_CUSTOMER_PRODUCTS"))) {
					?>
				...
				if (!empty(getDolGlobalString("MAIN_AUTO_OPEN_SELECT2_ON_FOCUS_FOR_SUPPLIER_PRODUCTS"))) {
					?>
				<script type="text/javascript">
					$(document).ready(function(){
						// On first focus on a select2 combo, auto open the menu (this allow to use the keyboard only)
		...
		$nbrows = ROWS_2;
		$enabled = (!empty(getDolGlobalString("FCKEDITOR_ENABLE_DETAILS")) ? getDolGlobalString("FCKEDITOR_ENABLE_DETAILS")  : 0);
		if (!empty(getDolGlobalString("MAIN_INPUT_DESC_HEIGHT"))) $nbrows = getDolGlobalString("MAIN_INPUT_DESC_HEIGHT");
		$toolbarname = 'dolibarr_details';
		if (!empty(getDolGlobalString("FCKEDITOR_ENABLE_DETAILS_FULL"))) $toolbarname = 'dolibarr_notes';
		$doleditor = new DolEditor('dp_desc', GETPOST('dp_desc', 'none'), '', (empty(getDolGlobalString("MAIN_DOLEDITOR_HEIGHT")) ? 100 : getDolGlobalString("MAIN_DOLEDITOR_HEIGHT") ), $toolbarname, '', false, true, $enabled, $nbrows, '98%');
		$doleditor->Create();
...
if (is_object($objectline)) {
	print $objectline->showOptionals($extrafields, 'edit', array('colspan'=>$coldisplay), '', '', empty(getDolGlobalString("MAIN_EXTRAFIELDS_IN_ONE_TD")) ? 0 : 1);
}

if ((!empty($conf->service->enabled) || ($object->element == 'contrat')) && $dateSelector && GETPOST('type') != '0') {	// We show date field if required
	print '<tr id="trlinefordates" class="oddeven">'."\n";
	if (!empty(getDolGlobalString("MAIN_VIEW_LINE_NUMBER"))) { print '<td></td>'; }
	print '<td colspan="'.($coldisplay - (empty(getDolGlobalString("MAIN_VIEW_LINE_NUMBER")) ? 0 : 1)).'">';
	...
	if (!empty($object->element) && $object->element == 'contrat') {
		...
		print $form->selectDate($date_start, 'date_start', empty(getDolGlobalString("MAIN_USE_HOURMIN_IN_DATE_RANGE")) ? 0 : 1, empty(getDolGlobalString("MAIN_USE_HOURMIN_IN_DATE_RANGE")) ? 0 : 1, 1, "addproduct", 1, 0);
		print ' '.$langs->trans('to').' ';
		print $form->selectDate($date_end, 'date_end', empty(getDolGlobalString("MAIN_USE_HOURMIN_IN_DATE_RANGE")) ? 0 : 1, empty(getDolGlobalString("MAIN_USE_HOURMIN_IN_DATE_RANGE")) ? 0 : 1, 1, "addproduct", 1, 0);
	};
	print '<script>';
	if (!$date_start) {
		if (!empty(getDolGlobalString("MAIN_DEFAULT_DATE_START_HOUR"))) {
			print 'jQuery("#date_starthour").val("'.getDolGlobalString("MAIN_DEFAULT_DATE_START_HOUR") .'");';
		}
		if (!empty(getDolGlobalString("MAIN_DEFAULT_DATE_START_MIN"))) {
			print 'jQuery("#date_startmin").val("'.getDolGlobalString("MAIN_DEFAULT_DATE_START_MIN") .'");';
		}
	}
	if (!$date_end) {
		if (!empty(getDolGlobalString("MAIN_DEFAULT_DATE_END_HOUR"))) {
			print 'jQuery("#date_endhour").val("'.getDolGlobalString("MAIN_DEFAULT_DATE_END_HOUR") .'");';
		}
		if (!empty(getDolGlobalString("MAIN_DEFAULT_DATE_END_MIN"))) {
...
<script>
	...
	$("#idprod, #idprodfournprice").change(function()
	{
		...
		<?php
		if (empty(getDolGlobalString("MAIN_DISABLE_EDIT_PREDEF_PRICEHT")) && empty($senderissupplier)) {
			?>
