 ...
 */
class pdf_comptage extends ModelePDFStock
{
	/**
	...
	public function __construct($db)
	{
		...
		$langs->loadLangs(array("main", "companies","operationorder@operationorder"));

		$this->db = $db;
		$this->name = "comptage";
		$this->description = $langs->trans("DocumentModelcomptagePDF");
		...
		$this->posxpicture = $this->posxtva - (empty(getDolGlobalInt("MAIN_DOCUMENTS_WITH_PICTURE_WIDTH")) ? 20 : getDolGlobalInt("MAIN_DOCUMENTS_WITH_PICTURE_WIDTH") ); // width of images
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->stock->dir_output) {
			...
			if (file_exists($dir)) {
				...
				if (!empty(getDolGlobalString("MAIN_GENERATE_DOCUMENTS_SHOW_FOOT_DETAILS"))) {
					$heightforfooter += 6;
				}
				...
				if (empty(getDolGlobalString("MAIN_DISABLE_FPDI")) && !empty(getDolGlobalString("MAIN_ADD_PDF_BACKGROUND"))) {
					$pagecount = $pdf->setSourceFile($conf->mycompany->dir_output.'/'.getDolGlobalInt("MAIN_ADD_PDF_BACKGROUND"));
				...
				$tab_top_newpage = (!getDolGlobalInt('MAIN_PDF_DONOTREPEAT_HEAD') ? 32 + $top_shift : 10);
				...
				if ($resql) {
					...
					for ($i = 0; $i < $nblines; $i++) {
						...
						if ($pageposafter > $pageposbefore && empty($showpricebeforepagebreak)) {
							...
							$pdf->writeHTMLCell($this->wref, 4, $curX, $curY, $outputlangs->convToOutputCharset($objp->ref), 0, 1, false, true, 'J', true);
							$nexY = $tab_top_newpage + $this->tabTitleHeight+4;
						}
						...
						$pdf->MultiCell($this->posxqty - $this->posxlabel - 0.8, 3, dol_trunc($productstatic->label, 40), 0, 'L');
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell($this->page_largeur - $this->marge_droite-$this->posxunit - 1, 2, $outputlangs->transnoentities("Commentaires"), '', 'C');
		}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey = "")
	{
		...
		if ($object->statut == 0 && (!empty(getDolGlobalString("STOCK_DRAFT_WATERMARK")))) {
			pdf_watermark($pdf, $outputlangs, $this->page_hauteur, $this->page_largeur, 'mm', getDolGlobalString("STOCK_DRAFT_WATERMARK"));
		}
