 ...
 */
class pdf_or_V1 extends ModelePDFOperationOrder
{
	var $db;
	var $name;
	var $description;
	...
	protected function _pagehead(&$pdf, $object, $showHeadDetails, $outputlangs, $titlekey = "PdfCustomOrderTitle")
	{
		...
		// Translations
		$outputlangs->loadLangs(array('operationorder@operationorder',"main", "bills", "propal", "orders", "companies"));
		...
		if ($showHeadDetails) {
			...
			dol_include_once('dolifleet/class/vehicule.class.php');
			$vehicule = new Vehicule($db); // use to get table of element
			...
			$input = 'km_on_creation';
			$outVehicule.= '<tr>';
			$outVehicule.= '<td class="titlefield"  ></td>';
			$outVehicule.= '<td ></td>';
			$outVehicule.= '<td >&nbsp;</td>';
			$outVehicule.= '<td class="titlefield"  >'.$langs->trans($object->fields['km_on_creation']['label']).'</td>';
			$outVehicule.= '<td ><strong>'.$object->showOutputFieldQuick('km_on_creation').'</strong></td>';
			$outVehicule.= '</tr>';
