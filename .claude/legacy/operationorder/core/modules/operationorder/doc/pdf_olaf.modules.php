 ...
 */
class pdf_olaf extends ModelePDFOperationOrder
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Olaf";
		$this->description = $langs->trans('PDFolafDescription');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->operationorder->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$heightforfooter = $this->marge_basse + (empty(getDolGlobalInt("MAIN_GENERATE_DOCUMENTS_SHOW_FOOT_DETAILS")) ? 5 : 10); // Height reserved to output the footer (value include bottom margin)
				...
				if ($notetoshow) {
					...
					if ($pageposafternote > $pageposbeforenote) {
						...
						while ($pagenb < $pageposafternote) {
							...
							$pagenb++;
							if (!empty($tplidx)) $pdf->useTemplate($tplidx);
							if (empty(getDolGlobalString("MAIN_PDF_DONOTREPEAT_HEAD"))) $this->_pagehead($pdf, $object, 0, $outputlangs);
				...
				for ($i = 0 ; $i < $nblines ; $i++) {
					...
					if ($this->getColumnStatus('ean')) {
						...
						$pdf->write1DBarcode($code, 'EAN13', $this->getColumnContentXStart('ean'), $curY, $this->getColumnContentWidth('ean'), $barCodeHeight, '', $style);
	 ...
	 *  @return	void
	 */
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey = "PdfCustomOrderTitle")
	{
		// phpcs:enable
	...
	public function defineColumnField($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$this->cols['ean'] = array(
			...
			'content' => array(
				'padding' => array(0, 5, 0, 5), // Like css 0 => top , 1 => right, 2 => bottom, 3 => left
			),
		);
		...
		$this->cols['photo'] = array(
			'rank' => $rank,
			'width' => (empty(getDolGlobalInt("MAIN_DOCUMENTS_WITH_PICTURE_WIDTH")) ? 20 : getDolGlobalInt("MAIN_DOCUMENTS_WITH_PICTURE_WIDTH") ), // in mm
			'status' => false,
			'title' => array(
				'textkey' => 'Photo',
				'label' => ' '
		...
		//      );
		//
		//      if (empty(getDolGlobalString("MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT") ) && empty(getDolGlobalString("MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN") ))
		//      {
