...
if (!empty($action)) {
	switch ($action) {
		case 'sync':
			...
			if (!empty(getDolGlobalString("MAIN_INFO_SOCIETE_COUNTRY"))) {
				list($id_country, $code_country) = explode(':', getDolGlobalString("MAIN_INFO_SOCIETE_COUNTRY"));
...
print dol_get_fiche_head($head, 'oojoursOff', $langs->trans("OperationOrderJoursOff"), -1, 'operationorder@operationorder');
