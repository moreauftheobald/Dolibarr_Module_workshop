...
$DailyPlanning = $or->getOperationOrderUserPlanningSchedule($date_lundi, $date_dimanche);
if (!is_array($DailyPlanning) && $DailyPlanning<0) {
	setEventMessages($or->error, $or->errors, 'errors');
}
$fk_groupuser = getDolGlobalInt("OPERATION_ORDER_GROUPUSER_DEFAULTPLANNING");
$planningUser = $or->getOperationOrderTUserPlanningFromGroup($fk_groupuser);

//FHE : Change min and max time (Ticket 0000025)
$minHour = "06:00";
$maxHour = "21:00";
...
elseif ($reshook > 1) $TSchedules = $hookmanager->resArray;
	...
	<form name="filters" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
		...
		<table width="100%">
			...
			<?php if (isModEnabled("multicompany")) {
				$sql = "SELECT rowid, label FROM ".$db->prefix()."entity WHERE active = 1";
...
if (!empty($TSchedules)) {
	...
	$TSchedulesIndispo = [];
	foreach ($TSchedules as $userSchdulId=>$sched) {
		foreach ($sched->schedule as $indexSched=>$dataSched) {
			if (isset($dataSched->text) && $dataSched->text == 'indispo') {
				//var_dump($TSchedules[$userSchdulId]);
				$TSchedulesIndispo[$userSchdulId][] = ['start'=>$dataSched->start, 'end'=>$dataSched->end];
				unset($TSchedules[$userSchdulId]->schedule[$indexSched]);
			}
		}
	}
	?>
	...
	<script type="text/javascript">
		...
		* trouvé ici : https://www.jqueryscript.net/time-clock/Simple-Daily-Schedule-Plugin-with-jQuery-and-jQuery-UI-Schedule.html */
		...
		$(function(){
			...
			var $sc = $("#schedule").timeSchedule({
				...
				onClick: function(node, data){ // quand on clique sur un événement
					...
					if (counterID)
					{
						...
						$.ajax({
							url: '<?php echo dol_buildpath('/operationorder/scripts/interface.php', 1); ?>?action=getScheduleInfos&token=<?php echo currentToken()?>',
							method: 'POST',
							data: {
				...
				onScheduleClick: function(node, data){ // quand on clique sur un endroit vide
					...
					if (userid != undefined)
					{
						...
						$.ajax({
							url: '<?php echo dol_buildpath('/operationorder/scripts/interface.php', 1); ?>?action=getCreateScheduleForm&token=<?php echo currentToken()?>',
							method: 'POST',
							data: {
								userid: userid,
								date: date,
			...
			let inspo =  <?php print json_encode($TSchedulesIndispo) ?>;
			$('#schedule').find('div.tl').each(function(index) {
				let userid =$(this).data('timeline');
				let divtime= $(this);
					$.each(inspo[userid], function(index, value) {
						//console.log(value.start,value.end,divtime.data('time'));
						if (value.start<=divtime.data('time') &&
							value.end>divtime.data('time')) {
							divtime.css("background-color", "lightgrey");
						}
					})
			});
		});
