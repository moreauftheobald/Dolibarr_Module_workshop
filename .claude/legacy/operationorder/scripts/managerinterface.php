...
if (empty($reshook) && !empty($action)) {
	if ($action == 'logged-status') {
		...
		if (empty(getDolGlobalString("OPERATION_ORDER_GROUPUSER_DEFAULTPLANNING"))) {
			$data['errorMsg'] = $langs->trans('ImcompleteSetup', $langs->transnoentities('OPERATION_ORDER_GROUPUSER_DEFAULTPLANNING'));
		} else {
			$userGroup = new UserGroupOperationOrder($db);
			$retgroup = $userGroup->fetch(getDolGlobalInt("OPERATION_ORDER_GROUPUSER_DEFAULTPLANNING"));
			if ($retgroup > 0) {
		...
		if (!empty($u)) {
			...
			if ($ret > 0) {
				...
				if (!empty($counter->fk_orDet)) {
					$sql = "SELECT oorder.ref FROM " . $db->prefix() . "operationorder oorder";
					$sql .= " INNER JOIN " . $db->prefix() . "operationorderdet ordet ON ordet.fk_operation_order = oorder.rowid";
					$sql .= " WHERE ordet.rowid = " . $counter->fk_orDet;
					//	$sql .= ' AND oorder.status IN ( SELECT s.rowid FROM ' . $db->prefix() . $sOperationOrderStatus->table_element . ' as s WHERE  or_pointable > 0 ) ';
		...
		$sql = "SELECT DISTINCT ooa.fk_operationorder FROM " . $db->prefix() . "operationorderaction ooa";
		$sql .= " INNER JOIN " . $db->prefix() . "operationorder oorder ON oorder.rowid=ooa.fk_operationorder";
			...
			while ($obj = $db->fetch_object($resql)) {
				...
				if ($oOrder->id) {
					$data['oOrders'][$i] = array(
						...
						'tooltips' => dol_string_nohtmltag($oOrder->getORTooltips(), 1),
						'ref' => $oOrder->ref,
					...
					if (isModEnabled("dolifleet")) {
						...
						if (isset($oOrder->fk_vehicule)) {
							$sqlVeh = "SELECT immatriculation FROM " . $db->prefix() . "dolifleet_vehicule";
							$sqlVeh .= " WHERE rowid = " . $oOrder->fk_vehicule;
		...
		if (!empty($OR->lines)) {
			...
			foreach ($OR->lines as $line) {
				if ($line->fk_product && !array_key_exists(intval($line->fk_product), $TPointable)) {
					...
					} elseif ($line->product->type == Product::TYPE_SERVICE) continue;

					$ProdErrors[$line->fk_product] = '';

					if (empty(getDolGlobalString("STOCK_SUPPORTS_SERVICES")) && $line->product->type == Product::TYPE_SERVICE && !$TPointable[$line->fk_product])
					...
					if (empty($line->product->fk_default_warehouse) && empty($line->fk_warehouse))
		...
		if ($ret > 0) {
			...
			if ($ret > 0 && $counter->fk_orDet > 0) {
				...
				if (!empty(getDolGlobalString("OPORDER_CHANGE_OR_STATUS_ON_STOP")) && !empty(getDolGlobalInt("OPODER_STATUS_ON_STOP")) && !$remaining) {
					list($changeReturn, $message) = changeORStatus($ordet->fk_operation_order, getDolGlobalInt("OPODER_STATUS_ON_STOP"));
		...
		if ($ret > 0) {
			...
			if ($retupd > 0) {
				if ($counter->fk_orDet > 0) {
					...
					$remaining = $counter->remainingCountersForOR($ordet->id);
					// changement de statut de l'OR de la ligne
					if (!empty(getDolGlobalString("OPORDER_CHANGE_OR_STATUS_ON_STOP")) && !empty(getDolGlobalString("OPODER_STATUS_ON_STOP")) && !$remaining) {
						list($changeReturn, $message) = changeORStatus($ordet->fk_operation_order, getDolGlobalInt("OPODER_STATUS_ON_STOP"));
						if ($changeReturn) $data['msg'] .= $message;
						else $data['errorMsg'] .= $message;
					}
	...
	} elseif ($action == 'startLineCounter') {
		$orBarcode = GETPOST('or_barcode');
		$orRef = substr($orBarcode, 2);
		...
		if (!$usr->id) {
		...
		} elseif ($OR->id != $line->fk_operation_order) {
			...
			if ($retSave > 0) {
				...
				// changement de statut de l'OR de la ligne
				if (!empty(getDolGlobalString("OPORDER_CHANGE_OR_STATUS_ON_START")) && !empty(getDolGlobalString("OPODER_STATUS_ON_START")) && !$remaining) {
					$OR->fk_user_meca = $usr->id;
					$OR->save($usr);
					list($changeReturn, $message) = changeORStatus($line->fk_operation_order, getDolGlobalInt("OPODER_STATUS_ON_START"));
		...
		if ($ret > 0) {
			...
			if ($ret > 0) {
				$alreadyUsed = array();
				$sql = "SELECT mvt.fk_product, SUM(mvt.value) as total FROM " . $db->prefix() . "stock_mouvement as mvt";
				...
				if (!empty($OR->lines)) {
					...
					foreach ($OR->lines as $line) {
						if ($line->fk_product == $prod->id) {
							$foundOnLineId = $line->id;
							$productWarehouse = $line->fk_warehouse;
							$prodTotalQty += $line->qty;
						}
					}

					if (!empty($foundOnLineId)) {
						if (empty(getDolGlobalString("STOCK_SUPPORTS_SERVICES")) && $prod->type == Product::TYPE_SERVICE) {
							$data['errorMsg'] = $langs->trans('ErrorStockMVTService');
						} else {
							if (empty($productWarehouse) && !empty($prod->fk_default_warehouse)) {
								$productWarehouse = $prod->fk_default_warehouse;
							}
							if (!empty($productWarehouse)) {
								// création de mouvement de stock
								$mvt = new MouvementStock($db);
								$mvt->origin_type = $OR->origin_type;
								$mvt->origin_id = $OR->id;
								$qty = 1;
								...
								if ($qtyAfterMvt > $prodTotalQty && !empty(getDolGlobalString("OPODER_CANT_EXCEED_SENT_QTY"))) {
									$data['errorMsg'] = $langs->trans('ErrorProductqtyExceded');
								} else {
									$result = $mvt->livraison($user, $prod->id, $productWarehouse, $qty, 0, $langs->trans('productUsedForOorder', $OR->ref));
								...
								$data['errorMsg'] = $langs->trans('ErrorNoWarehouseFound');
							}
						}
					} else {
						// le produit n'existe pas, on le crée si la conf est activée
						if (!empty(getDolGlobalString("OPODER_ADD_PRODUCT_IN_OR_IF_MISSING"))) {
							if (empty($productWarehouse) && !empty($prod->fk_default_warehouse)) {
								$productWarehouse = $prod->fk_default_warehouse;
							}

							if (empty($productWarehouse)) {
								$data['errorMsg'] = $langs->trans('ErrorCannotAddProductNoDefaultWarehouse', $prod->ref, $OR->ref);
							} else {
								$ret = $OR->addline('', 1, $prod->price, $productWarehouse, $prod->pmp, 0, 0, $prod->id);
								if ($ret > 0) {
...
/**
 * @param $or_id int or_id
 * @param $fk_status int status
 * @return array
 * @throws Exception
 */
