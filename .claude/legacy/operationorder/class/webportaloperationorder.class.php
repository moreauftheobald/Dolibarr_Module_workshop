 ...
 */
class WebPortalOperationorder extends OperationOrder
{
...
// affichage dans la baniere
		'rowid' => array('type' => 'int', 'label' => 'Id', 'enabled' => 0, 'position' => 110, 'notnull' => 1, 'visible' => 0, 'noteditable' => '1'),
		'ref'			  => array(
			'type'			 => 'varchar(128)',
			'label'			 => 'Ref',
			'enabled'		 => 1,
		...
		'date_creation'	  => array(
			'type'		  => 'datetime',
			'label'		  => 'DateCreationOperationOrder',
			'enabled'	  => 1,
			'position'	  => 1100,
			'notnull'	  => 1,
			'visible'	  => 4,
		...
		'km_on_creation'  => array(
			'type'	   => 'integer',
			'label'	   => 'km_on_creation',
			'enabled'  => 1,
			'position' => 5200,
			'notnull'  => 1,
			'visible'  => 1,
		...
		'status'		  => array(
			...
			'index'			=> 1,
			'arrayofkeyval' => array(
				-1 => 'OperationOrderStatusShortCanceled',
				0  => 'OperationOrderStatusShortDraft',
				1  => 'OperationOrderStatusShortValidated'
			)
		),
	);
	//public $rowid;
     ...
     */
    protected function getOperationorderStatic()
    {
        if (!$this->operationorder_static) {
            $this->operationorder_static = new OperationOrder($this->db);
        }
    ...
    public function getTooltipContentArray($params)
    {
        ...
        $datas['picto'] = img_picto('', $this->picto) . ' <u>' . $langs->trans("WebPortalOperationOrder") . '</u>';
     ...
     */
    public function getNomUrl($withpicto = 0, $option = '', $max = 0, $short = 0, $notooltip = 0, $save_lastsearch_value =
        -1, $addlinktonotes = 0, $target = '')
    {
        ...
        $result .= $linkstart;
        if ($withpicto) {
            $result .= img_object(($notooltip ? '' : $label), $this->picto,
    ...
    public function getKanbanView($option = '', $arraydata = null)
    {
        ...
        $return .= '<span class="info-box-ref inline-block tdoverflowmax150 valignmiddle">' . (method_exists($this,
                'getNomUrl') ? $this->getNomUrl() : $this->ref) . '</span>';
        $return .= '<input id="cb' . $this->id . '" class="flat checkforselect fright" type="checkbox" name="toselect[]" value="' . $this->id . '"' . ($selected
                ? ' checked="checked"' : '') . '>';
        ...
        if (property_exists($this, 'total_ht')) {
            $return .= '<div class="info-box-ref amount">' . price($this->total_ht, 0, $langs, 0, -1, -1,
                    $conf->currency) . ' ' . $langs->trans('HT') . '</div>';
        }
	 ...
	 */
	public function fetchWebOperationOrder($id, $loadChild = true, $ref = null)
	{
		$static = new OperationOrder($this->db);
...
//	{
//		// phpcs:enable
//        var_dump($this->getOperationorderStatic());exit;
//		return $this->getOperationorderStatic()->LibStatut($status, $billed, $mode, $donotshowbilled);
//	}
}
