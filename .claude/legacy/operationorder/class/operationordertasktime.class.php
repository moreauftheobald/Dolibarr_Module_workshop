...
class OperationOrderTaskTime extends SeedObject
{
	...
	public $fields = array(
		...
		'task_datehour_d' => array('type' => 'datetime', 'label' => 'DateD', 'enabled' => 1, 'position' => 170, 'notnull' => 1, 'visible' => 1, 'required' => 0),
	...
	/**
	 * @param int $orId ID du user dont on veut le compteur courant
	 * @return int|array
	 */
	public function fetchCourantCounterForOR($orId)
	{
		global $conf;

		$returnPointageToClose = [];

		$sql = "SELECT pointage.rowid FROM " . $this->db->prefix() . $this->table_element. " as pointage";
		$sql .= " INNER JOIN ".$this->db->prefix()."operationorderdet as ordt ON ordt.rowid=pointage.fk_orDet";
		$sql .= " AND ordt.fk_operation_order=".(int) $orId;
		$sql .= " WHERE pointage.task_datehour_f IS NULL";
		$sql .= " AND pointage.entity = " . $conf->entity;

		$resql = $this->db->query($sql);
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				while ($obj = $this->db->fetch_object($resql)) {
					$returnPointageToClose[] = $obj->rowid;
				}
				return $returnPointageToClose;
			} else return 0;
		} else {
			$this->error = $this->db->lasterror();
			$this->errors[] = $this->error;
	...
	public function remainingCountersForOR($ordet_id)
	{
		...
		$sql .= " FROM " . $this->db->prefix() . "operationordertasktime c";
		$sql .= " LEFT JOIN " . $this->db->prefix() . "operationorderdet ood ON ood.rowid = c.fk_orDet";
