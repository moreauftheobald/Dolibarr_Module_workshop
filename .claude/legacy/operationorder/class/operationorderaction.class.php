...
class OperationOrderAction extends SeedObject
{
	...
	public $fields = array(
		...
		'fk_operationorder' => array('type' => 'integer:OperationOrder:operationorder/class/operationorder.class.php:1:entity IN (0, __ENTITY__)','notnull' => 1, 'label' => 'OperationOrder', 'enabled' => 1, 'position' => 90, 'visible' => 1, 'foreignkey' => 'operationorder.rowid',),
	...
	public function fetchByOR($fk_operationorder)
	{
		...
		$sql = "SELECT rowid FROM " . $this->db->prefix() . "operationorderaction WHERE fk_operationorder = '" . $fk_operationorder . "'";
		$resql = $this->db->query($sql);

		if ($resql) {
			$ORA = new OperationOrderAction($this->db);
