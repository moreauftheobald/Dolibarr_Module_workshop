...
class OperationOrderJoursOff extends SeedObject
{
	...
	public function fetchAll($limit = 0, $loadChild = true, $TFilter = array())
	{
		...
		$sql = 'SELECT rowid FROM '.$this->db->prefix().$this->table_element.' WHERE 1=1';
		if (!empty($TFilter)) {
		...
		$sql.= " AND entity IN (0,".(! empty(isModEnabled("multicompany")) && ! empty($conf->multicompany->transverse_mode)?"1,":"").$conf->entity.")";
	...
	function alreadyExists()
	{
		...
		$sql="SELECT count(*) as 'nb'  FROM ".$this->db->prefix().$this->table_element."
	...
	function isOff($date)
	{
		...
		//on récupère toutes les dates de jours fériés existant
		$sql="SELECT count(*) as 'nb'  FROM ".$this->db->prefix().$this->table_element."
             WHERE entity IN (0,".(! empty(isModEnabled("multicompany")) && ! empty($conf->multicompany->transverse_mode)?"1,":"").$conf->entity.")
