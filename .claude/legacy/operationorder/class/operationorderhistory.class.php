...
class OperationOrderHistory extends SeedObject
{
	...
	public $fields = array(
		...
		'fk_operationorder' => array('type' => 'integer', 'label' => 'OperationOrderId', 'enabled' => 1, 'position' => 40, 'notnull' => 1, 'visible' => 1),
		'fk_operationorderdet' => array('type' => 'integer', 'label' => 'operationorderLineId', 'enabled' => 1, 'position' => 40, 'notnull' => 0, 'visible' => 1),
	...
	public function stockMvt($object, $product, $qty)
	{
		...
		$useradm->fetch(1);
		if (!empty($this->fk_operationorder)) {
	...
	public function compareAndSaveDiff($oldcopy, &$object)
	{
		...
		if (!empty($TDiff)) {
			...
			foreach ($TDiff as $keyDiff => $diff) {
				...
				if ($keyDiff == 'fields' || $keyDiff == 'oldcopy' || $keyDiff == 'error' || ($object->element == 'operationorderdet' && $keyDiff == 'label') || is_object($diff)) continue;
				if (!is_array($diff)) {
					if (!empty($object->fields[$keyDiff])) {
						$oldvalue = $oldcopy->showOutputFieldQuick($keyDiff);
						$newvalue = $object->showOutputFieldQuick($keyDiff);
					} elseif ($keyDiff == 'objStatus') {
					...
					$this->description .= $langs->transnoentitiesnoconv($object->fields[$keyDiff]['label']);
					$this->description .= ' : ' . $oldvalue . ' => ' . $newvalue . ' <br/>';
					if ($keyDiff === 'objStatus') {
						unset($object->fields['objStatus']);
					}
				}
				if ($keyDiff == 'array_options') {
					foreach ($diff as $keyExtra => $diffExtra) {
						$keyExtra = str_replace('options_', '', $keyExtra);
						$oldvalue = $extrafields->showOutputField($keyExtra, $diffExtra, '', $object->table_element);
						$newvalue = $extrafields->showOutputField($keyExtra, $object->array_options['options_' . $keyExtra], '', $object->table_element);
			...
			if (!empty($object->array_options)) {
				...
				foreach ($object->array_options as $keyExtra => $valExtra) {
					if (!empty($valExtra)) {
						$keyExtra = str_replace('options_', '', $keyExtra);
						$this->description .= '</br>';
						$this->description .= $langs->transnoentitiesnoconv($extralabels[$keyExtra]) . ' : ' . $extrafields->showOutputField($keyExtra, $valExtra, '', $object->table_enement);
					}
				}
			}
			$this->fk_operationorder = $object->id;
