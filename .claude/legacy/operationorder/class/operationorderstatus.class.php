...
class OperationOrderStatus extends SeedObject
{
	...
	//      ),
		...
		'update_vehicule_info' => array(
			'type'=>'boolean',
			'label'=>'UpdateVehiculeInfo',
			'enabled'=>1,
			'default'=>0,
			'visible'=> -1,
			'position'=>115,
			'required'=>0,
			'help'=>'VehiculeInfosDetails'
		),
	);
	...
	function fetch_statusUserGroupRights()
	{
		...
		$sql = 'SELECT rowid,code,fk_usergroup FROM '.$this->db->prefix().$groupRights->table_element;
	...
	function fetch_statusAllowed()
	{
		...
		$sql = 'SELECT rowid,fk_operationorderstatus_target FROM '.$this->db->prefix().$targetableStatus->table_element;
		$sql.= ' WHERE fk_operationorderstatus = '.$this->id;
	...
	public function fetchDefault($id, $force_entity = 0)
	{
		...
		if ($res>0) {
			...
			$sql = 'SELECT rowid FROM '.$this->db->prefix().$this->table_element.' WHERE status = 1 ';
	...
	function updateStatusUserGroupRight($replace = false)
	{
		...
		foreach ($this->TGroupRightsType as $field) {
			...
			if (!empty($TToAdd)) {
				...
				$sql = 'INSERT INTO '.$this->db->prefix().$groupRights->table_element;
			...
			if (!empty($TToDel) && $replace) {
				...
				$sql = 'DELETE FROM '.$this->db->prefix().$groupRights->table_element.' WHERE code = \''.$this->db->escape($code).'\' AND fk_usergroup IN ('.implode(',', $TToDel).')  AND fk_operationorderstatus = '.intval($this->id).';';
	...
	function updateStatusAllowed($replace = false)
	{
		...
		if (!empty($TToAdd)) {
			...
			$sql = 'INSERT INTO '.$this->db->prefix().$targetableStatus->table_element;
		...
		if (!empty($TToDel) && $replace) {
			...
			$sql = 'DELETE FROM '.$this->db->prefix().$targetableStatus->table_element.' WHERE fk_operationorderstatus_target IN ('.implode(',', $TToDel).')  AND fk_operationorderstatus = '.intval($this->id).';';
	...
	public function delete(User &$user, $notrigger = false)
	{
		...
		$groupRights = new OperationOrderStatusUserGroupRight($this->db);
		$sql = 'DELETE FROM `llx_operationorder_status_usergroup_rights` '.$this->db->prefix().$groupRights->table_element;
		$sql.= ' WHERE fk_operationorderstatus = '.$this->id;
		$resql = $this->db->query($sql);
		if (!$resql) {
		...
		$sql = 'DELETE FROM `llx_operationorder_status_usergroup_rights` '.$this->db->prefix().$tragetableStatus->table_element;
		$sql.= ' WHERE fk_operationorderstatus = '.$this->id;
		$resql = $this->db->query($sql);
		if (!$resql) {
	...
	static public function updateRank($rowid, $rank)
	{
		...
		$sql = 'UPDATE '.$db->prefix().$status->table_element.' SET rang = '.intval($rank);
